...
def post_universal(class_):
    try:
        ...
        del input_data['id']  # если будет передан id, который передавать не нужно
...
def get_by_id_universal(uid, class_, schema):
    try:
        result = db.session.query(class_).get_or_404(uid)
...
def delete_universal(uid, class_):
    try:
        item_to_delete = db.session.query(class_).get_or_404(uid)
    except Exception as e:
